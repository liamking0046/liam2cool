# Enhanced run_motion_control.ps1 for realistic motion transfer
param(
    ...
    [double]$FlowScale = 0.9,
    [double]$GlobalScale = 1.0,
    [double]$BlendStrength = 0.1,
    [int]$MaxFrames = 0,
    [ValidateSet('natural', 'cgi')]
    [string]$Look = 'natural',
    [double]$LookStrength = 0.55,
    [double]$MotionFocus = 0.75,
    [double]$HandBoost = 0.45,
    [double]$TemporalSmooth = 0.3,
    [double]$IdentityLock = 0.5,
    [double]$StructureLock = 0.6,
    [double]$LightingTransfer = 0.4,
    [double]$FlowMomentum = 0.5,
    [int]$QualityPasses = 3,
    [double]$UpperbodyStickiness = 0.85,
    [double]$MicroMotion = 0.35
)
...
# Use py launcher if 'python' fails
try {
    python @pythonArgs
} catch {
